...
class instance_Ip(Tk):
    ...
    def display_Ip(self):
        try:
            ...
            res_lsa, res_lpt, res_rma = code_source.lsa(
                M, D), code_source.lpt(M, D), code_source.rma(M, D)
            b = max([max(D), sum(D)/len(D)])
            ratio_lsa = max(res_lsa) / b
            ratio_lpt = max(res_lpt) / b
            ratio_rma = max(res_rma) / b

            texte = "Borne inferieure \"maximum\" = " + str(max(D)) + "\n" + "Borne inferieure \"moyenne\" = " + \
                    str(sum(D)/len(D)) + "\n" + "Resultat LSA = " + str(max(res_lsa)) + \
                    "\n" + "ratio LSA = " + str(ratio_lsa) + "\n" + "Resultat LPT = " + str(max(res_lpt)) + "\n" + \
                    "ratio LPT = " + str(ratio_lpt) + "\n" + "Resultat RMA = " + str(max(res_rma)) + "\n" + \
                    "ratio RMA = " + str(ratio_rma)
            tkinter.messagebox.showinfo("Resultats", texte)
            self.destroy()
        except ValueError:
            tkinter.messagebox.showerror(
                "Erreur", "Vous devez saisir un entier !")
            self.destroy()
...
class instance_Ir(Tk):
    def __init__(self):
        ...
        self.label2 = Label(self, text="Choisissez un nombre de taches :")
        self.entree2 = Entry(self, width=30)
        self.label3 = Label(
            self, text="Choisissez un nombre d'instances a generer :")
        self.entree3 = Entry(self, width=30)
        self.label4 = Label(
            self, text="Choisissez une duree de tache minimum :")
        self.entree4 = Entry(self, width=30)
        self.label5 = Label(
            self, text="Choisissez une duree de tache maximum :")
    ...
    def display_Ir(self):
        try:
            ...
            tkinter.messagebox.showerror(
                "Erreur", "Vous devez saisir un (des) entier(s) !")
